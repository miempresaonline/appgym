      ...
      <div className="max-w-7xl w-full mx-auto px-6 py-12 md:py-20 flex flex-col lg:flex-row items-center justify-between gap-16 lg:gap-24 z-10 relative">
        ...
        >
          ...
          <div className="text-center mb-10 relative z-10">
            <h2 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-br from-white to-white/50 drop-shadow-lg">Entrar a la cápsula</h2>
          </div>
            ...
            <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
              <div className="space-y-3">
                <div className="relative group">
                  <div className="absolute inset-0 rounded-[1.25rem] bg-gradient-to-r from-purple-500/20 to-orange-500/20 opacity-0 group-hover:opacity-100 transition-opacity blur-md" />
                  <input 
                    type="email" 
                    placeholder="tucorreo@ejemplo.com" 
                    value={email}
                    onChange={(e) => setEmail(e.target.value)}
                    className="w-full relative bg-black/40 border border-white/10 rounded-[1.25rem] px-5 py-4 text-white placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-white/40 focus:border-white/40 focus:bg-black/60 transition-all shadow-inner backdrop-blur-xl"
                  />
                </div>
                <div className="relative group">
                  <div className="absolute inset-0 rounded-[1.25rem] bg-gradient-to-r from-purple-500/20 to-orange-500/20 opacity-0 group-hover:opacity-100 transition-opacity blur-md" />
              ...
              <motion.button 
                whileHover={{ scale: 1.02, y: -2, boxShadow: "0 10px 40px -10px rgba(255,255,255,0.4)" }}
                whileTap={{ scale: 0.98 }}
                className="w-full mt-2 relative flex items-center justify-center gap-2 bg-white text-black font-extrabold py-4 px-6 rounded-[1.25rem] shadow-[0_0_20px_rgba(255,255,255,0.2)] hover:shadow-[0_0_30px_rgba(255,255,255,0.6)] transition-all duration-300 disabled:opacity-50 overflow-hidden group"
              >
                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-black/10 to-transparent translate-x-[-100%] group-hover:animate-[shine_1.5s_ease-in-out_infinite]" />
                <span className="relative z-10 tracking-wider text-lg">INICIAR SESIÓN</span>
                <ArrowRight size={20} className="relative z-10 group-hover:translate-x-1 transition-transform" />
              </motion.button>
            </form>

            <p className="text-center text-white/50 text-sm mt-6 font-medium">
              ¿Aún no tienes cuenta? <a href="#" className="text-white hover:text-orange-400 underline decoration-white/30 underline-offset-4 transition-colors">Únete ahora</a>
            </p>
          </div>
        </motion.div>
      </div>
